...
Object mapping includes localized Program/merchant content, the immutable public progress-asset URL, state, and one opaque W3 membership QR code. The visible hierarchy is Google-native logo/issuer and program title, native QR, then one 1032:812 hero containing the selected stamp composition and concise localized reward copy on the same card background. There is no separate front reward row, numeric loyalty balance, or customer account name. The complete reward and status remain available in pass details. Google owns final typography and spacing. Add to Google Wallet actions are signed JWT links with bounded claims and allowed origins; the JWT references the stored object and does not embed the QR secret.
