...
const screenshotDirectory = resolve(process.argv[2] ?? "artifacts/handoff-w2-round-5/screenshots");
const output = resolve(
  process.argv[3] ?? "artifacts/handoff-w2-round-5/w2-round5-contact-sheet.png",
);
const files = (await readdir(screenshotDirectory))
  .filter((name) => /^\d+-r5-.*\.png$/u.test(name))
  .sort((left, right) => left.localeCompare(right, "en", { numeric: true }));

if (!files.length) throw new Error("No Round 5 screenshots were found.");
  ...
  composites.push({
    input: Buffer.from(`
      <svg width="${tileWidth}" height="${captionHeight}">
        ...
        <text x="18" y="54" font-family="Arial, sans-serif" font-size="18" fill="#f8dfd4">
