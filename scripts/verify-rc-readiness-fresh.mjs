...
const reviewDir = path.join(root, "artifacts", "rc-readiness-review");
...
const isolatedDbName = `waflo_rc_fresh_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
...
async function main() {
  ...
  let manifestMd = `# WAFLO RC VISUAL QA MANIFEST\n\n`;
  ...
  await writeFile(path.join(reviewDir, "RC-VISUAL-MANIFEST.md"), manifestMd, "utf8");
  await writeFile(
    path.join(reviewDir, "rc-metrics.json"),
