...
import { expectedImageSizes } from "../../apps/apple-pass-builder-service/src/protobuf.js";
import {
...
describe("Apple Pass Builder migration", () => {
  it("keeps legacy primary fields empty so strip artwork remains unobscured", () => {
    ...
    const migrated = mapAppleStoreCardPosterPass(input, configuration, "a".repeat(43));
    ...
    expect(migrated.storeCard.primaryFields).toEqual([]);
    expect(migrated.storeCard.secondaryFields).toContainEqual(
      expect.objectContaining({ key: "reward", value: rewardSummary }),
    );
  ...
  it("preserves installed-pass identity, update authentication, QR payload, colors, and Store Card semantics", () => {
    const token = "a".repeat(43);
    const legacy = mapAppleStoreCard(baseMembership, configuration, token);
    const migrated = mapAppleStoreCardPosterPass(baseMembership, configuration, token);
    ...
    // Pass Builder preserves the installed-pass identity and uses the same
    // native Store Card front field mapping as the standalone generator.
    ...
    expect(allFieldValues(migrated.storeCard)).toMatchObject({
      progress: "3/8",
  ...
  it("emits Poster Generic and Store Card fallback from the same personalization", () => {
    const pass = mapAppleStoreCardPosterPass(baseMembership, configuration, "a".repeat(43));
  ...
  it("covers representative current-platform golden personalizations without inventing tier fields", () => {
    ...
    for (const golden of cases) {
      ...
      expect(pass.storeCard, golden.name).toBeDefined();
      expect(pass.posterGeneric, golden.name).toBeDefined();
      if (golden.name === "arabic") {
        expect(allFieldValues(pass.storeCard)).toMatchObject({
          member: "سارة",
  ...
  it("sends bounded structured personalization and correctly sized PNG assets to the private adapter", async () => {
    ...
    expect(parsed.images).not.toHaveProperty("thumbnail");
    expect(() =>
      parsePassBuilderRequest({
        ...parsed,
        images: {
          ...parsed.images,
          thumbnail: parsed.images.strip,
        },
      }),
    ).toThrow("images.thumbnail is unsupported");
    expect(() =>
      parsePassBuilderRequest({
        ...parsed,
  ...
  it("uses the Store Card strip transport and has no thumbnail dependency", async () => {
    const request = await requestForMembership(baseMembership);
    expect(request.images).toHaveProperty("strip");
    expect(request.images).not.toHaveProperty("thumbnail");
    expect(expectedImageSizes.strip).toEqual([375, 144]);
    expect(expectedImageSizes).not.toHaveProperty("thumbnail");
  });
