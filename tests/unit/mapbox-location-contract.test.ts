...
describe("Mapbox-backed exact business location contract", () => {
  ...
  it("keeps click, touch, current-location, RTL, reverse-geocode, and attribution behavior explicit", () => {
    ...
    expect(picker).toContain("marker.setLngLat([longitude, latitude])");
    expect(picker).toContain("if (!marker.getElement().isConnected) marker.addTo(map)");
    expect(picker).toContain("markerElement.hidden = true");
    expect(picker).toContain('markerElement.setAttribute("role", "button")');
    expect(picker).toContain("scrollZoom: false");
    expect(picker).toContain("doubleClickZoom: false");
    expect(picker).toContain("map.touchZoomRotate.disableRotation()");
    ...
    expect(picker).toContain("enableHighAccuracy: false");
    expect(picker).toContain("Promise.any([fastRequest, preciseRequest])");
