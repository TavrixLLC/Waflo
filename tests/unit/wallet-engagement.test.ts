...
describe("Wallet provider-native nearby relevance", () => {
  it("maps Apple coordinates, relevantText, and requested maximum without changing pass identity", () => {
    ...
    expect(pass.storeCard.auxiliaryFields.find((field) => field.key === "status")).toMatchObject({
      changeMessage: "%@",
      key: "status",
      value: "Active",
    });
    expect(pass.storeCard.backFields.find((field) => field.key === "status")).toBeUndefined();
  });
...
describe("Wallet Engagement input safety", () => {
  ...
  it("normalizes every campaign to both Wallet providers without exposing provider choice", () => {
    ...
    expect(() => walletCampaignCreateSchema.parse({ ...validCampaign, providers: [] })).toThrow();
  });
